...
//
//  Objetivo: cuando un pedido con TARJETA se paga vía Wompi (domicilio o retiro),
//  registrar las estadísticas de venta + comisión Wompi (3.5%).
//
//  Flujo:
...
// ════════════════════════════════════════════════════════════════
//  Helper: registra estadísticas de venta de un pedido con tarjeta Wompi.
//  Idempotente: si ya tiene statsRecorded o paymentStatus 'paid', no hace nada.
// ════════════════════════════════════════════════════════════════
async function recordCardPaymentStats(orderId, wompiTxId) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    const comisionWompi = round2(baseCobro * WOMPI_FEE_PCT);
